        ...
        Alamofire.request(YAHOO_QUERY, encoding: URLEncoding.queryString).validate().responseJSON { response in
            ...
            case .success(let value):
                ...
                for (_,subJson):(String, JSON) in cities {
                    if let city = subJson["location"]["city"].string, let temperature = subJson["item"]["condition"]["temp"].string, let latitude = subJson["item"]["lat"].string, let longitude = subJson["item"]["long"].string {
